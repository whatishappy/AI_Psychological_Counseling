...
app.post('/api/consultations', async (req, res) => {
    try {
        ...
        // 确定要使用的AI模型类型
        let modelType = aiService_1.AIModelType.MOCK;
        const modelEnv = process.env.AI_MODEL_TYPE;
        if (modelEnv === 'glm') {
            modelType = aiService_1.AIModelType.GLM;
        }
        else if (modelEnv === 'glm-4v') {
            modelType = aiService_1.AIModelType.GLM_4V;
        }
        // 调用AI模型获取回复
        const aiResponse = await (0, aiService_1.callAIModel)(user_query, modelType);
        const response = {
